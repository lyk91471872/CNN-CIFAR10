...
def create_transform_from_config(config_list):
    ...
    # Add selected transforms
    for transform_config in config_list:
        ...
        # Create the transform using the v2 transforms module
        transform_class = getattr(v2, transform_name)
        transform = transform_class(**transform_params)
        transforms_list.append(transform)
    
    # Add standard transforms (ToTensor and Normalize)
...
def train_with_augmentation(transform, epochs, verbose=True):
    ...
    # Create custom dataset with the specified transform
    ...
    # Disable early stopping in the pipeline for a fixed number of epochs
    pipeline.early_stopping.patience = epochs * 2  # Set to a large value to prevent early stopping
