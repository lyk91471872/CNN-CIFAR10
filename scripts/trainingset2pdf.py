import os
from PIL import Image, ImageDraw, ImageFont
from tqdm import tqdm
import numpy as np

# Import from project root
import config as conf
from dataset import load_cifar_batch

class CIFAR10TrainingDatasetRaw:
    """
    CIFAR-10 training dataset that returns raw images without normalization or augmentation.
    
    Returns a tuple (raw_image, index) where raw_image is a PIL Image.
    """
    def __init__(self, file_path, max_images=None):
        batch = load_cifar_batch(file_path)
        self.data = batch[b'data']
        self.labels = batch[b'labels']
        
        # Reshape data to (N, 32, 32, 3)
        self.data = self.data.reshape(-1, 3, 32, 32).transpose(0, 2, 3, 1)
        
        # Limit the number of images if specified
        if max_images is not None:
            self.data = self.data[:max_images]
            self.labels = self.labels[:max_images]
        
    def __len__(self):
        return len(self.data)
    
    def __getitem__(self, idx):
        img = self.data[idx]
        # Convert to PIL Image directly without normalization
        pil_img = Image.fromarray(img)
        return pil_img, idx

def trainingset_to_pdf(dataset, output_pdf, use_grayscale=False):
    """
    Exports images from CIFAR10 training dataset directly to a PDF file without saving individual PNG files.

    Parameters:
        dataset: a CIFAR10TrainingDatasetRaw instance
        output_pdf (str): the PDF file path where images will be saved
        use_grayscale (bool): whether to convert images to grayscale
    """
    # Create output directory if it doesn't exist
    output_dir = os.path.dirname(output_pdf)
    if output_dir:
        os.makedirs(output_dir, exist_ok=True)

    num_images = len(dataset)
    if num_images == 0:
        print("Dataset is empty.")
        return

    # Prepare list for pages (each page will contain 100 images)
    pages = []
    images_per_page = 100
    grid_size = 10  # 10x10 grid per page
    page_width = page_height = 320 * grid_size  # 3200 x 3200 pixels per page

    # Process images in chunks of 100
    for i in tqdm(range(0, num_images, images_per_page), desc="Processing pages"):
        # Create a blank white page
        page = Image.new("RGB", (page_width, page_height), "white")

        # Process the next 100 images (or fewer if at the end)
        for j in range(min(images_per_page, num_images - i)):
            try:
                img, idx = dataset[i + j]
                label = dataset.labels[i + j]  # Get the label for this image

                # Resize image to 320x320 using nearest neighbor interpolation
                img = img.resize((320, 320), Image.NEAREST)

                # Apply grayscale if requested
                if use_grayscale:
                    img = img.convert("L").convert("RGB")
                    text_color = "magenta"
                else:
                    text_color = "black"

                # Draw the image id and label at the bottom left corner
                draw = ImageDraw.Draw(img)
                try:
                    font = ImageFont.truetype("bedstead-boldcondensed.otf", 24)
                except IOError:
                    # Fallback to default font if specified font not available
                    font = ImageFont.load_default()

                # Include both ID and class label
                class_names = ['airplane', 'automobile', 'bird', 'cat', 'deer', 
                              'dog', 'frog', 'horse', 'ship', 'truck']
                text = f'ID: {idx} | Class: {class_names[label]}'

                # Calculate text dimensions
                try:
                    bbox = font.getbbox(text)
                    text_width = bbox[2] - bbox[0]
                    text_height = bbox[3] - bbox[1]
                except AttributeError:
                    # For older PIL versions that don't have getbbox
                    text_width, text_height = font.getsize(text)

                padding = 5
                # Position: a few pixels from the left and bottom edge
                text_position = (padding, 320 - text_height - padding)
                draw.text(text_position, text, fill=text_color, font=font)

                # Determine grid position for this image
                row = j // grid_size
                col = j % grid_size
                x = col * 320
                y = row * 320

                page.paste(img, (x, y))
            except Exception as e:
                print(f"Error processing image {i + j}: {e}")

        pages.append(page)

    # Save all pages to a single PDF file
    if pages:
        pages[0].save(output_pdf, save_all=True, append_images=pages[1:])
        print(f"PDF successfully saved as '{output_pdf}'")
    else:
        print("No pages were created.")

if __name__ == '__main__':
    # Use only the first training batch (data_batch_1)
    training_batch_path = os.path.join(conf.DATA_DIR, 'data_batch_1')
    output_pdf_path = os.path.join(conf.SCRIPTS_OUTPUT_DIR, "training_images.pdf")
    
    # Create dataset from the first training batch
    training_dataset = CIFAR10TrainingDatasetRaw(training_batch_path)
    
    print(f"Processing {len(training_dataset)} images from training batch 1...")
    trainingset_to_pdf(training_dataset, output_pdf_path, use_grayscale=False) 